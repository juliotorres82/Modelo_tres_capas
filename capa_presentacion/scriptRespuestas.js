...
fetch('http://localhost:3000/peticion/respuestas')
  .then((response) => {
    if (!response.ok) {
    ...
    respuestas.forEach((respuesta) => {
      ...
      card.innerHTML = `
        <div class="card-body">
          ...
          <button class="btn btn-primary"><a style="text-decoration: none; color: inherit;" href="http://localhost:3000/actualizar?id=${respuesta.id}" data-method="GET">Editar</a></button>
          <button class="btn btn-danger"><a style="text-decoration: none; color: inherit;" href="http://localhost:3000/peticion/eliminar?id=${respuesta.id}" data-method="GET">Eliminar</a></button>
        </div>
      `;
